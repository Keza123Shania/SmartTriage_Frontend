...
/* Mock triage nurses — will come from backend later */
const MOCK_NURSES = [
  { id: 'N001', name: 'Nurse Uwimana Marie' },
  { id: 'N002', name: 'Nurse Habimana Jean' },
  { id: 'N003', name: 'Nurse Mukiza Alice' },
  { id: 'N004', name: 'Nurse Niyonzima Patrick' },
  { id: 'N005', name: 'Nurse Ishimwe Grace' },
];
...
export function EntryRegistration() {
  ...
  const selectedNurseName = useMemo(() => {
    const nurse = MOCK_NURSES.find((n) => n.id === formData.assignedNurseId);
    return nurse?.name || '';
  ...
  const handleSubmit = () => {
    ...
    const patient = addPatient({
      ...
      arrivalMode: formData.arrivalMode as ArrivalMode,
      mobility: (formData.mobility as Mobility) || undefined,
      referringFacility: formData.referringFacility || undefined,
      referralDocumentFile: formData.referralDocumentFile || undefined,
      weight: formData.weight ? parseFloat(formData.weight) : undefined,
      assignedNurseId: formData.assignedNurseId,
      assignedNurseName: selectedNurseName,
      contactPerson: formData.contactPersonName
        ? {
            name: formData.contactPersonName,
            phone: formData.contactPersonPhone,
            relationship: formData.contactPersonRelationship,
          }
        : undefined,
      guardian: isPediatric && formData.guardianName
        ? {
            name: formData.guardianName,
            phone: formData.guardianPhone,
            relationship: formData.guardianRelationship,
            nationalId: formData.guardianNationalId || undefined,
          }
        : undefined,
      registeredBy: formData.assignedNurseId,
    });
