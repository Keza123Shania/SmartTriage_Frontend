...
import {
  ...
  FileText, Stethoscope, ClipboardList, Bell, Wifi, WifiOff,
...
import { MOCK_MONITORED } from '@/modules/monitoring/ConstantMonitoring';
...
export function VitalMonitoring() {
  ...
  // Use store patient if available, otherwise fall back to mock data
  const patient = storePatient || MOCK_MONITORED.find(p => p.id === patientId);
  
  // Use vitals from store, or from mock patient's currentVitals
  const mockPatient = MOCK_MONITORED.find(p => p.id === patientId);
  const currentVitals = vitals || (mockPatient?.currentVitals ? {
    ...mockPatient.currentVitals,
    timestamp: new Date(),
  } : null);
  
  // Start vital simulation (with IoT device management)
  const { deviceStatus } = useVitalSimulator(patientId!);
  ...
  const vitalHistory: VitalHistoryData | null = currentVitals ? (
    mockPatient?.vitalHistory 
      ? {
          hr: mockPatient.vitalHistory.map((v: any) => ({ timestamp: v.time, value: v.hr })),
          rr: mockPatient.vitalHistory.map((v: any) => ({ timestamp: v.time, value: v.rr })),
          spo2: mockPatient.vitalHistory.map((v: any) => ({ timestamp: v.time, value: v.spo2 })),
          bp: mockPatient.vitalHistory.map((v: any) => ({ timestamp: v.time, value: v.sbp })),
          temp: mockPatient.vitalHistory.map((v: any) => ({ timestamp: v.time, value: v.temp })),
          ecg: mockPatient.vitalHistory.map((v: any) => ({ timestamp: v.time, value: v.ecg })),
          glucose: mockPatient.vitalHistory.map((v: any) => ({ timestamp: v.time, value: v.glucose })),
        }
      : {
 ...
 return (
    <div className={`min-h-screen p-6 ${isDark ? 'bg-[#020b14]' : 'bg-gradient-to-br from-gray-50 to-gray-100'}`}>
      <div className="max-w-7xl mx-auto">
          ...
          <div className={`rounded-xl shadow-md p-4 ${isDark ? glassCard + ' border border-white/10' : 'bg-white border border-gray-200'}`}>
            <div className="flex items-start justify-between">
              <div className="flex items-center gap-4">
                <div className="w-14 h-14 bg-gradient-to-br from-slate-800 to-cyan-500 rounded-xl flex items-center justify-center text-white text-xl font-bold">
                  {patient.fullName.split(' ').map(n => n[0]).join('')}
                </div>
                <div>
                  <h1 className={`text-xl font-bold mb-1 ${isDark ? 'text-white' : 'text-gray-900'}`}>{patient.fullName}</h1>
                  <div className={`flex items-center gap-3 text-sm ${isDark ? 'text-slate-400' : 'text-gray-600'}`}>
                    ...
                    {patient.isPediatric && (
                      <>
                        <span>•</span>
                        <span className={`px-2 py-0.5 rounded-full text-xs font-semibold ${isDark ? 'bg-pink-500/15 text-pink-300 border border-pink-500/30' : 'bg-pink-100 text-pink-700'}`}>
                          Pediatric
                        </span>
                      </>
                    )}
              ...
              <div className="text-right">
                ...
                <div className={`text-xs ${isDark ? 'text-slate-400' : 'text-gray-600'}`}>
                  TEWS: <span className={`font-bold text-base ${isDark ? 'text-white' : 'text-gray-900'}`}>{patient.tewsScore}</span>
                </div>
              </div>
            </div>
            ...
            <div className={`mt-4 pt-4 border-t ${isDark ? 'border-white/10' : 'border-gray-200'}`}>
              <div className="flex items-start gap-2">
                <ClipboardList className={`w-4 h-4 mt-0.5 ${isDark ? 'text-slate-400' : 'text-gray-600'}`} />
                <div>
                  <div className={`text-xs font-semibold mb-0.5 ${isDark ? 'text-slate-400' : 'text-gray-600'}`}>Chief Complaint</div>
                  <div className={`text-sm font-medium ${isDark ? 'text-white' : 'text-gray-900'}`}>{patient.chiefComplaint}</div>
                </div>
              </div>
            </div>
          </div>
        ...
        {/* Live Status Bar */}
        <div className={`mb-4 rounded-xl p-3 shadow-sm border ${isDark ? 'bg-green-500/10 border-green-500/20' : 'bg-gradient-to-r from-green-50 to-emerald-50 border-green-200'}`}>
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              <div className="flex items-center gap-2">
                <div className={`w-2 h-2 rounded-full animate-pulse ${
                  deviceStatus.hasActiveDevice ? 'bg-green-500' : 'bg-red-500'
                }`}></div>
                <span className={`text-sm font-semibold ${
                  ...
                  {deviceStatus.hasActiveDevice ? 'Live Monitoring Active' : 'Device Disconnected'}
                </span>
              </div>
              ...
              {deviceStatus.activeDeviceCount > 0 && (
                <span className={`text-xs font-medium px-2 py-0.5 rounded-full ${isDark ? 'text-emerald-300 bg-emerald-500/15' : 'text-emerald-600 bg-emerald-100'}`}>
                  {deviceStatus.activeDeviceCount} device{deviceStatus.activeDeviceCount > 1 ? 's' : ''} streaming
                </span>
              )}
            </div>
            <div className="flex items-center gap-2">
              {deviceStatus.lowestBattery < 30 && (
                <span className={`flex items-center gap-1 text-xs font-medium px-2 py-0.5 rounded-full border ${isDark ? 'text-amber-300 bg-amber-500/15 border-amber-500/30' : 'text-amber-600 bg-amber-50 border-amber-200'}`}>
                  ...
                  Battery {Math.round(deviceStatus.lowestBattery)}%
                </span>
              )}
              <button
        ...
        {/* Tab Navigation */}
        <div className="mb-4">
          <div className={`flex gap-1.5 rounded-xl p-1.5 shadow-sm border ${isDark ? glassCard + ' border-white/10' : 'bg-white border-gray-200'}`}>
            {[
              { id: 'vitals', label: 'Current Vitals', icon: Heart },
              { id: 'trends', label: 'Vital Trends', icon: TrendingUp },
              { id: 'alerts', label: 'Alerts & Interventions', icon: Bell },
              { id: 'devices', label: 'IoT Devices', icon: MonitorSmartphone },
