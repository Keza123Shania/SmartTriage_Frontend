...
export function Dashboard() {
  ...
  const stats = {
    ...
    waiting: mockPatients.filter((p) => p.triageStatus === 'WAITING').length,
    inTriage: mockPatients.filter((p) => p.triageStatus === 'IN_TRIAGE').length,
    triaged: mockPatients.filter((p) => p.triageStatus === 'TRIAGED').length,
    critical: mockPatients.filter((p) => p.category === 'RED').length,
    pediatric: mockPatients.filter((p) => p.isPediatric).length,
    averageTEWS:
      mockPatients.filter((p) => p.tewsScore !== undefined).length > 0
        ? (
          mockPatients.reduce((sum, p) => sum + (p.tewsScore || 0), 0) /
          mockPatients.filter((p) => p.tewsScore !== undefined).length
        ).toFixed(1)
        : '0.0',
    categoryBreakdown: {
      ...
      <div className="p-5 space-y-4">
        ...
        <div className="relative z-10 flex items-center justify-between animate-fade-in">
          <div className="relative">
            ...
            <p className="text-slate-500 text-sm mt-0.5 font-medium">Real-time patient management & triage overview</p>
          </div>
          <div className="flex items-center gap-3">
            ...
            <div className="relative">
              ...
              <span className="absolute -top-1.5 -right-1.5 min-w-[20px] h-5 bg-red-500 text-white text-[10px] font-bold rounded-full flex items-center justify-center px-1 shadow-lg pointer-events-none animate-pulse">
                {unreadAlerts > 0 ? unreadAlerts : 5}
              </span>

              {/* Notification Dropdown */}
              {showNotifications && (
                <div className="absolute right-0 top-full mt-3 w-96 rounded-2xl shadow-2xl z-[9999] overflow-hidden animate-scale-in" style={glassDropdown}>
                  <div className={`flex items-center justify-between px-5 py-3.5 border-b ${isDark ? 'border-white/10' : 'border-gray-200/80'}`} style={{ background: isDark ? 'rgba(8, 47, 73, 0.95)' : 'rgba(248,250,252,1)' }}>
                    <span className={`text-sm font-bold tracking-wide ${text.heading}`}>Notifications</span>
                  ...
                  <div className="max-h-80 overflow-y-auto">
                    ...
                    {[
                      { id: 'sn1', icon: 'critical', title: 'Critical Vital Signs', message: 'Jean Mugabo — BP dropped to 85/55 mmHg', time: '3 min ago' },
                      { id: 'sn2', icon: 'warning', title: 'Triage Override', message: 'Dr. Uwimana requested override for Marie I.', time: '12 min ago' },
                      { id: 'sn3', icon: 'info', title: 'New Patient', message: 'Emmanuel Habimana registered and awaiting triage', time: '25 min ago' },
                      { id: 'sn4', icon: 'critical', title: 'Sepsis Risk Alert', message: 'AI model detects 78% sepsis risk — Claudine U.', time: '38 min ago' },
                      { id: 'sn5', icon: 'success', title: 'Triage Completed', message: 'Patrick Niyonzima triaged as YELLOW — Bay 3', time: '52 min ago' },
                    ].map((n, i) => (
                      <div key={n.id} className={`px-5 py-3.5 border-b ${isDark ? 'border-white/5 hover:bg-white/5' : 'border-gray-100/80 hover:bg-gray-50/80'} cursor-pointer transition-all duration-200 animate-fade-in-up`} style={{ animationDelay: `${i * 0.05}s` }}>
                        <div className="flex items-start gap-3">
                          {n.icon === 'critical' ? (
                            <div className={`w-8 h-8 rounded-xl bg-gradient-to-br ${isDark ? 'from-red-500/20 to-red-600/10' : 'from-red-50 to-red-100/80'} flex items-center justify-center flex-shrink-0 shadow-sm`}>
                              <AlertTriangle className="w-4 h-4 text-red-500" />
                            </div>
                          ) : n.icon === 'warning' ? (
                            <div className={`w-8 h-8 rounded-xl bg-gradient-to-br ${isDark ? 'from-amber-500/20 to-amber-600/10' : 'from-amber-50 to-amber-100/80'} flex items-center justify-center flex-shrink-0 shadow-sm`}>
                              <AlertCircle className="w-4 h-4 text-amber-500" />
                            </div>
                          ) : n.icon === 'success' ? (
                            <div className={`w-8 h-8 rounded-xl bg-gradient-to-br ${isDark ? 'from-emerald-500/20 to-emerald-600/10' : 'from-emerald-50 to-emerald-100/80'} flex items-center justify-center flex-shrink-0 shadow-sm`}>
                              <CheckCircle className="w-4 h-4 text-emerald-500" />
                            </div>
                          ) : (
                            <div className={`w-8 h-8 rounded-xl bg-gradient-to-br ${isDark ? 'from-cyan-500/20 to-cyan-600/10' : 'from-accent-100 to-accent-200/80'} flex items-center justify-center flex-shrink-0 shadow-sm`}>
                              <Bell className={`w-4 h-4 ${isDark ? 'text-cyan-400' : 'text-primary-700'}`} />
                            </div>
                          )}
                          <div className="flex-1 min-w-0">
                            <p className={`text-sm font-bold ${isDark ? 'text-white' : 'text-gray-900'} tracking-wide`}>{n.title}</p>
                            <p className={`text-xs ${isDark ? 'text-slate-300' : 'text-gray-600'} mt-1 leading-relaxed`}>{n.message}</p>
                            <p className={`text-xs ${isDark ? 'text-slate-500' : 'text-gray-400'} mt-1.5 font-medium`}>{n.time}</p>
                          </div>
                        </div>
                      </div>
                    ))}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
            ...
            >
              <div className="flex items-center justify-between mb-4">
                <div>
                  ...
                  <p className={`text-xs ${text.body} font-medium mt-0.5`}>AI monitoring signals</p>
                </div>
                {alerts.length > 0 && (
                  <span className="text-[11px] font-bold bg-red-500 text-white px-2.5 py-1 rounded-full shadow-sm animate-pulse">{alerts.length}</span>
                )}
              </div>
              <div className="space-y-2">
                {alerts.slice(0, 4).map((alert) => (
                  ...
                  >
                    <div className="flex items-start gap-2.5">
                      ...
                      <div className="flex-1 min-w-0">
                        <p className="text-[13px] font-semibold text-slate-800 leading-snug truncate">{alert.message}</p>
                        <p className="text-[11px] text-slate-400 mt-1">{formatDistanceToNow(alert.timestamp, { addSuffix: true })}</p>
                      </div>
                ...
                {alerts.length === 0 && (
                  <div className="text-center py-8">
                    ...
                    <p className="text-[11px] text-slate-400 mt-1">All patients are stable</p>
                  </div>
                )}
              </div>
            </div>
            ...
            >
              ...
              <div className="space-y-3">
                {[
                  { label: 'Ambulance Arrivals', value: mockPatients.filter((p) => p.arrivalMode === 'AMBULANCE').length },
                  { label: 'Walk-in Patients', value: mockPatients.filter((p) => p.arrivalMode === 'WALK_IN').length },
