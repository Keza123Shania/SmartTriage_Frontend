...
import {
  ...
  Wind, Eye, Activity, User, FileText, Users,
...
export function AdultTriageForm() {
  ...
  const [drNotifiedAt, setDrNotifiedAt] = useState<Date | null>(null);
  const [drAttendedAt, setDrAttendedAt] = useState<Date | null>(null);
  ...
  const handleNotifyDoctor = useCallback(() => { setDrNotifiedAt(new Date()); }, []);
  const handleDoctorAttended = useCallback(() => { setDrAttendedAt(new Date()); }, []);
      ...
      <div className="p-4 lg:p-6 space-y-4">
        ...
        <div className="rounded-2xl overflow-hidden shadow-xl" style={glassCard}>
          ...
          <div className="p-4">
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
              ...
              <div><label className={labelCls}>Next of Kin</label><input type="text" value={nextOfKin} onChange={(e) => setNextOfKin(e.target.value)} className={inputCls} /></div>
              <div><label className={labelCls}>Phone Number</label><input type="tel" value={phoneNumber} onChange={(e) => setPhoneNumber(e.target.value)} placeholder="+250 ..." className={inputCls} /></div>
            </div>
        ...
        <div className="rounded-2xl p-4" style={glassCard}>
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
            ...
            <div className="space-y-3">
              {(categoryResult.category === 'RED' || categoryResult.category === 'ORANGE') && (
                <>
                  <div className="flex items-center gap-1.5 mb-1">
                    <Bell className="w-3.5 h-3.5 text-red-500" /><h3 className="text-xs font-bold text-slate-800">Doctor Notification</h3>
                    <span className="text-[9px] font-bold bg-red-100 text-red-700 px-1.5 py-0.5 rounded-full">{categoryResult.category} Patient</span>
                  </div>
                  <div className="flex gap-2">
                    <button onClick={handleNotifyDoctor} disabled={!!drNotifiedAt} className={`flex-1 py-2 rounded-lg text-[11px] font-semibold transition-all flex items-center justify-center gap-1.5 ${drNotifiedAt ? 'bg-green-100 text-green-700 border border-green-200' : 'bg-red-600 text-white hover:bg-red-700 shadow-sm'}`}>
                      <Bell className="w-3.5 h-3.5" />{drNotifiedAt ? `Notified ${drNotifiedAt.toLocaleTimeString()}` : 'Notify Doctor'}
                    </button>
                    <button onClick={handleDoctorAttended} disabled={!drNotifiedAt || !!drAttendedAt} className={`flex-1 py-2 rounded-lg text-[11px] font-semibold transition-all flex items-center justify-center gap-1.5 ${drAttendedAt ? 'bg-green-100 text-green-700 border border-green-200' : drNotifiedAt ? 'bg-cyan-600 text-white hover:bg-cyan-500 shadow-sm' : 'bg-slate-100 text-slate-400 cursor-not-allowed'}`}>
                      <CheckCircle className="w-3.5 h-3.5" />{drAttendedAt ? `Attended ${drAttendedAt.toLocaleTimeString()}` : 'Doctor Attended'}
                    </button>
                  </div>
                </>
              )}
            ...
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-3">
                {triageFinished ? (
                  ...
                  <>
                    {(categoryResult.category === 'RED' || categoryResult.category === 'ORANGE') && !drNotifiedAt && (
                      <span className="text-[10px] text-red-600 font-bold animate-pulse bg-red-50 px-2.5 py-1.5 rounded-lg border border-red-200">⚠ Notify doctor first</span>
                    )}
                    <button onClick={handleFinishTriage} disabled={!canFinish || ((categoryResult.category === 'RED' || categoryResult.category === 'ORANGE') && !drNotifiedAt)} className="px-5 py-2 bg-gradient-to-r from-cyan-600 to-slate-800 text-white rounded-xl text-xs font-bold hover:shadow-xl transition-all disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-1.5 hover:-translate-y-0.5">
